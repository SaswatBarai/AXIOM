...
function NotifItem({ n, onRead }: { n: AppNotification; onRead: () => void }) {
  ...
  return (
    <div
      className={`flex items-start gap-3 px-4 py-3 cursor-pointer hover:bg-bg-hover transition-colors
        ${!n.readAt ? "bg-brand/10 border-l-2 border-brand" : ""}`}
      onClick={onRead}
    >
      <div className="flex-shrink-0 mt-0.5 p-1.5 rounded-lg bg-bg-elevated">
        <Icon size={13} className="text-brand" />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-xs font-medium text-text-primary">{TYPE_LABEL[n.type] ?? n.type}</p>
      ...
      {!n.readAt && (
        <div className="flex-shrink-0 mt-1.5 w-1.5 h-1.5 rounded-full bg-brand" />
      )}
    </div>
  );
}
...
export function NotificationBell() {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-10 w-80 rounded-xl border border-border-subtle bg-bg-card shadow-2xl z-50 overflow-hidden">
          {/* Header */}
            ...
            <div className="flex items-center gap-2">
              {unreadCount > 0 && (
                <button
                  type="button"
                  onClick={markAllRead}
                  className="flex items-center gap-1 text-[11px] text-brand hover:text-brand-hover transition-colors cursor-pointer"
                >
                  <CheckCheck size={12} /> Mark all read
                </button>
              )}
              <button type="button" onClick={() => setOpen(false)} className="text-text-muted hover:text-text-primary cursor-pointer">
                <X size={14} />
              </button>
            </div>
          </div>
          ...
          <div className="max-h-80 overflow-y-auto divide-y divide-border-subtle/50">
            {notifications.length === 0 ? (
              <div className="py-10 text-center text-sm text-text-muted">No notifications yet</div>
            ) : (
              notifications.map((n) => (
