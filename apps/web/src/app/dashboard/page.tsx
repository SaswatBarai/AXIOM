...
function StatCard({ 
  ...
  return (
    <Card className="border border-border-subtle bg-bg-card/40 p-5 shadow-lg relative overflow-hidden group hover:border-brand/30 transition-all duration-300">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="w-8 h-8 rounded-lg bg-bg-elevated border border-border-subtle flex items-center justify-center text-text-secondary group-hover:text-brand transition-colors duration-200">
          <Icon className="w-4 h-4" />
        </div>
      </div>
      {sub && <p className="text-[10px] text-text-muted mt-2 font-medium">{sub}</p>}
    </Card>
  );
}
    ...
    <div className="h-full overflow-y-auto bg-bg-base">
      <div className="p-6 md:p-8 max-w-5xl space-y-8">
        ...
        >
          ...
          <p className="text-text-secondary text-sm">{contextualSubtext}</p>
        </motion.div>
        ...
        {!isLoading && completionPct < 100 && (
          <motion.div
            ...
            className="relative"
          >
            <Card className="border border-border-subtle bg-bg-card/40 p-4.5 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-2">
                  ...
                  {missingProfileFields.length > 0 && (
                    <button 
                      ...
                      className="text-[10px] text-text-secondary hover:text-brand font-bold flex items-center gap-1 cursor-pointer transition-colors animate-none"
                    >
                ...
                <AnimatePresence>
                  {showMissingTip && missingProfileFields.length > 0 && (
                    ...
                    >
                      <div className="pt-2.5 border-t border-border-subtle mt-2.5 flex flex-wrap gap-2 animate-in fade-in duration-200">
                        <span className="text-[9px] text-text-muted font-bold w-full uppercase tracking-wider">Pending Tasks:</span>
                        {missingProfileFields.map((field) => (
                          <span key={field.label} className="text-[9px] font-bold text-text-secondary bg-bg-elevated border border-border-subtle px-2 py-0.5 rounded-md">
        ...
        >
          ...
          <StatCard 
            ...
            value={`${completionPct}%`} 
            sub="Integrity score" 
            color={completionPct === 100 ? "text-emerald-500" : "text-text-primary"} 
            icon={TrendingUp} 
          />
        </motion.div>
        ...
        >
          <h2 className="text-[10px] font-bold text-text-muted uppercase tracking-widest">Quick Actions</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {QUICK_ACTIONS.map(({ href, label, description, icon: Icon }) => (
              <Link
                key={href}
                href={href}
                className="group flex items-start gap-4 p-4.5 rounded-xl border border-border-subtle bg-bg-card/20 hover:bg-bg-hover/30 hover:border-border-medium transition-all duration-300"
              >
                <div className="w-9 h-9 rounded-lg bg-bg-elevated border border-border-subtle flex items-center justify-center shrink-0 group-hover:bg-bg-hover group-hover:text-brand transition-colors">
                  <Icon size={17} className="text-text-secondary group-hover:text-brand" />
                </div>
                ...
                <ArrowRight size={15} className="ml-auto mt-0.5 text-text-muted group-hover:text-text-secondary shrink-0 transition-colors" />
              </Link>
            ))}
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center justify-between">
            <h2 className="text-[10px] font-bold text-text-muted uppercase tracking-widest">Recommended for you</h2>
            {resumes.length > 0 && (
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1.5">
                  <button 
                    ...
                    className="w-7 h-7 rounded-lg bg-bg-elevated border border-border-subtle hover:border-border-medium flex items-center justify-center text-text-secondary hover:text-text-primary transition-all cursor-pointer"
                    title="Scroll left"
                  >
                    <ChevronLeft size={14} />
                  </button>
          ...
          {resumes.length === 0 ? (
            ...
            <Card className="border border-dashed border-border-subtle bg-bg-card/20 p-8 text-center max-w-xl mx-auto rounded-2xl space-y-4">
              <FileText size={32} className="mx-auto text-brand" />
              <div className="space-y-1">
                <h3 className="text-sm font-bold text-text-primary">Unlock personalized recommendations</h3>
            ...
            <div className="text-text-muted text-xs py-4 flex items-center gap-1.5">
            ...
            >
              {recommendedJobs.map((job) => (
                <div
                  key={job.id}
                  className="shrink-0 w-72 rounded-xl border border-border-subtle bg-bg-card/20 p-4.5 flex flex-col justify-between hover:border-border-medium transition-colors duration-200 snap-start"
                >
                  <div>
                    <div className="flex items-center justify-between gap-2 mb-3">
                      ...
                      <span className="text-[9px] text-text-muted font-bold uppercase tracking-wider">{job.source}</span>
                    </div>
                    <h3 className="text-xs font-bold text-text-primary truncate">{job.title}</h3>
                    <p className="text-xs text-text-secondary truncate mt-0.5">{job.company}</p>
                    ...
                    {job.matchedSkills && job.matchedSkills.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-1">
                        {job.matchedSkills.slice(0, 3).map((skill) => (
                          <span 
                            key={skill} 
                            className="text-[8px] font-extrabold text-text-secondary bg-bg-elevated border border-border-subtle px-1.5 py-0.5 rounded-md"
                          >
                            ✓ {skill}
                          </span>
                        ))}
                        {job.matchedSkills.length > 3 && (
                          <span className="text-[8px] font-bold text-text-muted px-0.5 pt-0.5">
                    ...
                    {job.missingSkills && job.missingSkills.length > 0 && (
                      <div className="mt-1 flex flex-wrap gap-1">
                        {job.missingSkills.slice(0, 2).map((skill) => (
                          <span 
                            key={skill} 
                            className="text-[8px] font-bold text-text-muted bg-bg-card border border-border-subtle px-1.5 py-0.5 rounded-md"
                    ...
                    <p className="text-xs text-text-secondary mt-3.5 line-clamp-2 leading-relaxed">{job.description}</p>
                  </div>
                  <div className="mt-4.5 pt-3.5 border-t border-border-subtle flex items-center justify-between text-xs">
                    <span className="text-[10px] text-text-secondary font-medium">{job.location}</span>
                    <a
                      href={job.sourceUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-[10px] font-bold text-text-primary hover:text-brand hover:underline flex items-center gap-0.5 transition-colors"
                    >
        ...
        <motion.div
          ...
          className="rounded-xl border border-border-subtle bg-bg-card/40 p-6 space-y-4"
        >
          <div className="flex items-center justify-between border-b border-border-subtle pb-3">
            <div className="flex items-center gap-2">
              <TrendingUp size={16} className="text-brand" />
              <h3 className="text-xs font-bold text-text-primary uppercase tracking-wider">Application Funnel</h3>
            </div>
          ...
          {applications.length === 0 ? (
            <div className="text-center py-6 space-y-1">
              <p className="text-xs font-bold text-text-secondary">No applications tracked yet</p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: "Saved Roles", count: funnelData.SAVED, color: "bg-bg-elevated/50 text-text-secondary border-border-subtle hover:border-border-medium" },
                { label: "Applied", count: funnelData.APPLIED, color: "bg-brand/10 text-brand border-brand/20 hover:border-brand/40" },
                { label: "Interviewing", count: funnelData.INTERVIEW, color: "bg-amber-500/10 text-amber-500 border-amber-500/20 hover:border-amber-500/40" },
                { label: "Offers", count: funnelData.OFFER, color: "bg-emerald-500/10 text-emerald-500 border-emerald-500/20 hover:border-emerald-500/40" },
              ].map((step) => (
                <div 
                  key={step.label} 
